...
FIXTURE_SCENARIOS = frozenset(
    {"native", "bridge", "missing", "authentication-required", "malformed"}
)
...
SECRET_PATTERNS = re.compile(
    r"token|secret|password|api[_-]?key|credential|bearer", re.I
)
KNOWN_FIELDS = {
    "schema_version",
    "platform",
...
def validate_request(raw: Any) -> list[str]:
    """Return validation errors without raising for malformed top-level values."""
    ...
    if platform == "macos":
        if domain != "unsupported":
            errors.append("macos requires execution_domain 'unsupported'")
        ...
        errors.append(
            f"execution_domain {domain!r} is not compatible with platform {platform!r}"
        )

    distro = raw.get("distro")
    if domain == "windows-wsl":
        if not isinstance(distro, str) or not distro.strip():
            errors.append("windows-wsl requires a distro")
        elif distro.lower() in {"docker-desktop", "docker-desktop-data"}:
            errors.append("Docker Desktop is not a development distro")
    ...
    if not isinstance(agents, list) or not agents:
        ...
        strings = [item for item in agents if isinstance(item, str)]
        if len(agents) != len(strings) or len(strings) != len(set(strings)):
            errors.append("requested_agents contains duplicates or non-string values")
        for agent in agents:
            if agent not in KNOWN_AGENTS:
                errors.append(f"unsupported agent: {agent!r}")

    if raw.get("operation") not in SUPPORTED_OPERATIONS:
        errors.append(f"unsupported operation: {raw.get('operation')!r}")
    if raw.get("install_missing_only") is not True:
        errors.append("install_missing_only must be true")
    if not isinstance(raw.get("native_preference"), bool):
        errors.append("native_preference must be boolean")
    if not isinstance(raw.get("bridge_permission"), bool):
        errors.append("bridge_permission must be boolean")
    if raw.get("posture") not in {"fixture", "live"}:
        errors.append("posture must be fixture or live")

    scenario = raw.get("fixture_scenario")
    if raw.get("posture") == "fixture" and scenario not in FIXTURE_SCENARIOS:
        errors.append("fixture posture requires a known fixture_scenario")
    if raw.get("posture") == "live" and scenario is not None:
        errors.append("fixture_scenario is not valid for live posture")

    evidence = raw.get("evidence_output_dir")
    if evidence is not None:
        if not isinstance(evidence, str) or not evidence:
            errors.append("evidence_output_dir must be a non-empty string")
        elif re.match(r"^[A-Za-z]:[\\/]", evidence) or evidence.startswith(
            ("/", "\\", "~")
        ):
            errors.append("evidence_output_dir must be portable and relative")

    unknown = set(raw) - KNOWN_FIELDS
    if unknown:
        errors.append(f"unknown field(s): {', '.join(sorted(unknown))}")
    if SECRET_PATTERNS.search(json.dumps(raw)):
        errors.append("request contains secret-like material")
    return errors


def _agent_result(
    agent: str, domain: str, backend: str, scenario: str = "native"
) -> dict[str, Any]:
    ...
    return {
        ...
        "reason_code": (
            ...
            if backend == "missing"
            else "fixture-resolution"
        ),
        "commands": {
            "canonical": agent,
            "native": f"{agent}_native",
            "bridge": f"{agent}_win",
        },
    }
...
def run_fixture(request: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for agent in request.get("requested_agents", []):
        if scenario == "missing":
            backend = "missing"
        elif scenario == "authentication-required":
            backend = "native"
        elif scenario == "bridge":
            backend = "bridge" if request.get("bridge_permission") else "missing"
        else:
            backend = "native"
        agents[agent] = _agent_result(
            agent, str(request.get("execution_domain")), backend, scenario
        )
    ...
    return build_result(
        request,
        agents,
        "action-required" if action else "pass",
        EXIT_ACTION_REQUIRED if action else EXIT_SUCCESS,
    )
...
def _probe(command: Path) -> tuple[bool, str | None]:
    try:
        result = subprocess.run(
            [str(command), "--agent-switchboard-probe"],
            capture_output=True,
            text=True,
            timeout=10,
            check=False,
...
def _run_wsl(
    distro: str, arguments: list[str], timeout: int = 10
...
def _probe_wsl_wrapper(
    distro: str, wrapper: str, allow_bridge: bool = False
) -> tuple[bool, str | None]:
    ...
    command = (
        ...
        f'{bridge}exec "$HOME/.local/agent-switchboard/bin/{wrapper}" '
        "--agent-switchboard-probe"
    )
    return _completed_probe(_run_wsl(distro, ["bash", "-lc", command]))


def _run_live_windows_wsl(
    ...
    if request.get("operation") == "install-missing":
        ...
        install_command = (
            'exec "$1" --destination "$HOME/.local/agent-switchboard/bin" '
            "--execution-domain windows-wsl"
        )
        ...
        if completed.returncode != 0:
            raise RuntimeError("WSL wrapper installation failed")
        installed = True
    ...
    for agent in request.get("requested_agents", []):
        ...
        if backend != "missing":
            canonical_ok, version = _probe_wsl_wrapper(
                distro, agent, allow_bridge=backend == "bridge"
            )
        result = _agent_result(
            agent, "windows-wsl", backend if canonical_ok else "missing"
        )
        result.update(
            version=version,
            smoke_status=(
                "command-probe-passed" if canonical_ok else "command-probe-failed"
            ),
...
def _probe_local_agent(
    agent: str, request_domain: str, bridge_permission: bool
) -> dict[str, Any]:
    managed = _find_managed_command(agent)
    if managed:
        native_ok, native_version = _probe(managed)
        if native_ok:
            result = _agent_result(agent, request_domain, "native")
            result.update(
                version=native_version,
                smoke_status="command-probe-passed",
                action_required=False,
                reason_code="command-probe-passed",
            )
            return result

    if bridge_permission:
        bridge_command = shutil.which(f"{agent}_win")
        if bridge_command:
            bridge_ok, bridge_version = _probe(Path(bridge_command))
            if bridge_ok:
                result = _agent_result(agent, "windows-native", "bridge")
                result.update(
    ...
    result = _agent_result(agent, request_domain, "missing")
    result.update(
        version=None,
        smoke_status="command-probe-failed",
        action_required=True,
        reason_code="command-probe-failed" if managed else "agent-missing",
    )
    return result
...
def run_live(request: Mapping[str, Any], repo_root: Path) -> dict[str, Any]:
    ...
    if request.get("operation") == "install-missing":
        ...
        completed = subprocess.run(
            ["bash", str(installer), "--destination", str(destination)],
            capture_output=True,
            text=True,
            check=False,
            timeout=30,
        ...
        if completed.returncode != 0:
            raise RuntimeError("Agent wrapper installation failed")
        installed = True

    request_domain = str(request.get("execution_domain"))
    bridge_permission = bool(request.get("bridge_permission"))
    agents = {
        agent: _probe_local_agent(agent, request_domain, bridge_permission)
        for agent in request.get("requested_agents", [])
    }
    action = any(item["action_required"] for item in agents.values())
    result = build_result(
