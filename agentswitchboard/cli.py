...
def main(argv: list[str] | None = None) -> int:
    ...
    if args[0] == "--version":
        print("agentswitchboard 0.2.0")
        return EXIT_SUCCESS
    if args[0] == "--supported-profiles":
        print("windows-native windows-wsl linux-native")
        return EXIT_SUCCESS
    if args[0] == "--supported-operations":
        print("inventory install-missing repair-check smoke")
        return EXIT_SUCCESS
    if args[0] == "--supported-agents":
    ...
    if not args:
        print("Missing REQUEST.json argument", file=sys.stderr)
        return EXIT_INVALID_REQUEST
    ...
    try:
        result = (
            run_fixture(raw)
            if raw["posture"] == "fixture"
            else run_live(raw, Path(__file__).resolve().parents[1])
        )
