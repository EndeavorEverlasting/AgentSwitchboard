...
if ($installStart -lt 0 -or $installEnd -le $installStart) { [void]$failures.Add('runtime-recovery-install-block-missing') }
 ...
 foreach ($token in @('installer="$(mktemp)"','raw.githubusercontent.com/anomalyco/opencode/__installer_commit__/install','grep -fq ''install_dir=$home/.opencode/bin''','grep -fq -- ''--no-modify-path''','bash "$installer" --no-modify-path')) { if (-not $installBlock.Contains($token)) { [void]$failures.Add("runtime-recovery-installer-contract:$token") } }
}
...
foreach ($token in @('immutable','forced-kill','final shim-creation health gate','raw stderr','inherited path')) { if (-not $manifestProofRule.Contains($token)) { [void]$failures.Add("manifest-runtime-recovery-proof-rule-missing:$token") } }
