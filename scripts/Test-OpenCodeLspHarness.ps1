...
foreach ($token in @('installtimeoutseconds = 180','https://opencode.ai/install','agentswitchboard\bin\opencode.cmd','invoke-boundedprocess','timeout --signal=term','-mode inspect','exit $lastexitcode')) { if (-not $runtimeRecoveryRouterLower.Contains($token)) { [void]$failures.Add("runtime-recovery-router-contract:$token") } }
...
foreach ($token in @('opencode_not_found','never emit the same failing gate as its own next action','same-state retry commands are insufficient','do not delegate opencode_not_found to broad technician setup','instead of hanging indefinitely')) { if (-not $failureWorkflow.Contains($token)) { [void]$failures.Add("failure-recovery-progress-contract:$token") } }
