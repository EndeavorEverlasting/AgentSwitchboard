...
try {
    ...
    $ids = @($registry.classifications | ForEach-Object { [string]$_.classificationId })
    foreach ($id in @('native-consistent','declared-wsl-consistent','shim-shadowing-native','parent-child-divergence','state-command-drift','unresolved-runtime-identity')) {
        Check ($ids -contains $id) "registry/classification/$id" 'classification is missing'
    }
...
foreach ($token in @('## Working','## Broken','## Missing','## Known traps','## Salvage disposition','## Validation','## Proof ceiling','AgentSwitchboard\bin\opencode.cmd','%APPDATA%\npm','Shared routing is deferred')) {
...
foreach ($forbidden in @('SetEnvironmentVariable(','npm install --global','Invoke-WebRequest','Remove-Item -LiteralPath $shim','wsl --install')) {
    Check (-not $deployableText.Contains($forbidden)) "forbidden/$forbidden" 'diagnostic harness embeds workstation mutation'
}

if ($failures.Count -eq 0) {
    & python (Join-Path $RootPath 'tests/test_opencode_runtime_resolution_harness.py')
    Check ($LASTEXITCODE -eq 0) 'python/contracts' 'semantic contracts failed'
}
if ($failures.Count -eq 0) {
    ...
    try {
        ...
        if (Test-Path -LiteralPath $snapshotPath -PathType Leaf) {
            ...
            Check ($snapshot.requestedSurface -eq 'unknown') 'reporter/requested-surface' 'requested surface was rewritten'
            Check ($null -eq $snapshot.parentResolution) 'reporter/parent-null' 'repository-only status invented parent resolution'
            ...
            Check ($snapshot.repositoryHarnessStatus -eq 'ready') 'reporter/readiness' 'tracked leaf harness is incomplete'
        }
    } catch {
