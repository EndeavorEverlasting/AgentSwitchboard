...
function Fail([string]$Name, [string]$Detail) { $script:failures.Add("$Name :: $Detail"); Write-Host "[FAIL] $Name :: $Detail" }
...
foreach ($path in $jsonFiles) {
    try { Get-Content -LiteralPath (Join-Path $root $path) -Raw | ConvertFrom-Json | Out-Null; Pass "json/$path" }
    catch { Fail "json/$path" $_.Exception.Message }
}
...
if ($manifest.rules.bindingDigestRequiredForVerification -eq $true) { Pass 'manifest/binding-digest-gate' } else { Fail 'manifest/binding-digest-gate' 'must be true' }
...
foreach ($needle in @('actor-mismatch','actor-verified','--requested-actor','--selected-actor','--actual-actor','--evidence','--expected-binding-sha256','binding digest mismatch','validate_binding')) {
...
foreach ($needle in @('bind-selected-actor','execute-through-selected-actor','verify-actual-actor','no silent actor substitution','BINDING_SHA256','expected-binding-sha256')) {
...
if ($skill.Contains('--expected-binding-sha256')) { Pass 'skill/pinned-binding-digest' } else { Fail 'skill/pinned-binding-digest' 'missing' }
...
if ($registry.Contains('routingNeedles')) { Pass 'integration/routing-needles' } else { Fail 'integration/routing-needles' 'missing deterministic actor route needles' }
...
$statusReporter = Get-Content -LiteralPath (Join-Path $root 'tooling/harness/operational/Get-OperationalHarnessStatus.py') -Raw
if ($statusReporter.Contains('routingNeedles')) { Pass 'integration/status-reporter-routing' } else { Fail 'integration/status-reporter-routing' 'status reporter does not consume registered routing needles' }
