...
*   **LSP-02** `in-progress` — ASQ-005 fresh-TUI vs headless `20260912T194619Z-e3f423df`. Floor on `main` (PR #163 @ `24cce9e`; ancestor of current tip); cloud/Linux runtime sprint **UNPROVEN**; **live TUI observation owned only by Admin Box 1** on `%USERPROFILE%\dev\AgentSwitchBoard-Live`.
...
Next: **Admin Box 1** runs ASQ-005 / LSP-02 live TUI smoke from canonical Live (`Join-Path $env:USERPROFILE 'dev\AgentSwitchBoard-Live'`). Parallel track: ASQ-015 FirstMate interop refresh after ADR sync. ASQ-008/009 stay blocked until ASQ-005 PASS. See `handoff.nextCommand` in `.plan.json` (public-safe; no OneDrive/private paths).
