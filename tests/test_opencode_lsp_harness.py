...
MANDATORY = {
 'manifest.json','codebase-map.json','workflows.json','artifact-registry.json','operator-report.template.md','Invoke-OpenCodeLspWorkstationSetup.ps1',
 'hooks/Invoke-OpenCodeLspPreCommit.ps1','hooks/Invoke-OpenCodeLspPrePush.ps1'
}
class OpenCodeLspHarnessTests(unittest.TestCase):
 ...
 def test_artifacts_are_local_and_untracked(self):
  ...
  self.assertFalse(data['tracked']); self.assertIn('%LOCALAPPDATA%', data['defaultRunRoot'])
 ...
 def test_ci_and_hooks_route_validation(self):
  ...
  self.assertIn('tests.test_opencode_lsp_harness',ci); self.assertIn('Test-OpenCodeLspHarness.ps1',ci); self.assertIn('git diff --check',ci)
