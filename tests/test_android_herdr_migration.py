...
def tracked(path: str) -> Path:
    ...
    result = subprocess.run(["git", "-C", str(ROOT), "ls-files", "--error-unmatch", "--", path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=False)
...
def main() -> None:
    ...
    assert manifest["candidate"]["nativeAndroidSourceBuildDecision"] == "BLOCKED_UNSUPPORTED_PLATFORM_FALLBACK"
    assert manifest["candidate"]["linuxMuslPrebuiltProbeDecision"] == "EXECUTION_PROBE_APPROVED_NO_INSTALL"
    assert manifest["candidate"]["runtimeCompatibilityNextGate"] == "exact-device-prebuilt-execution-identity"
    ...
    required_gates = {
        ...
        "source-bound-runtime-compatibility-reviewed",
        "exact-device-prebuilt-execution-identity-observed",
    ...
    for token in ("KEEP_TMUX_HERDR_NOT_INSTALLED", "KEEP_TMUX_HERDR_BINARY_NOT_HEALTHY", "HERDR_BINARY_CANDIDATE_ONLY", "proof_level=binary-readiness-only", "live-server-detach-reattach-and-agent-state-proof"):
    ...
    compat = json.loads(tracked("tooling/profiles/android/harness/herdr/upstream-runtime-compatibility.json").read_text(encoding="utf-8"))
    assert compat["source"]["releaseCommit"] == "346411fa21afd297f5ed3b3fa56f9e3fbf7654b7"
    assert compat["compatibility"]["linuxMuslPrebuiltOnTermux"]["buildTarget"] == "aarch64-unknown-linux-musl"
    assert compat["reviewDecision"] == "EXECUTION_PROBE_APPROVED_NO_INSTALL"
    assert compat["migrationDecision"] == "KEEP_TMUX"
    ...
    contract = subprocess.run(["bash", str(ROOT / "Test-AgentSwitchboard-Android-Herdr.sh"), "contract"], cwd=ROOT, text=True, capture_output=True, check=False)
    ...
    compat_contract = subprocess.run(["python", str(BASE / "Probe-HerdrPrebuiltCompatibility.py"), "contract"], cwd=ROOT, text=True, capture_output=True, check=False)
    assert compat_contract.returncode == 0, compat_contract.stderr
    assert "HERDR_PREBUILT_COMPATIBILITY_CONTRACT=PASS" in compat_contract.stdout

    guide = tracked("docs/workstation/android-herdr-migration.md").read_text(encoding="utf-8")
    for token in ("experimental candidate", "Linux `aarch64`", "does **not** currently claim Android/Termux support", "Do **not** uninstall tmux", "bash Test-AgentSwitchboard-Android-Herdr.sh evidence", "same-device", "Proof ceiling"):
