...
class TestOpenCodeCwdIndependentBootstrap(unittest.TestCase):
    ...
    def test_manifest_registers_location_free_operator_entrypoint(self) -> None:
        ...
        self.assertEqual(20, manifest["schemaVersion"])
        self.assertEqual(
    ...
    def test_windows_smoke_normalizes_git_and_powershell_paths(self) -> None:
        ...
        self.assertIn("$canonicalOrigin = ($canonical.TrimEnd('/') -replace '\\.git$','')", text)
        self.assertIn("$actualOrigin = ($origin.TrimEnd('/') -replace '\\.git$','')", text)
        self.assertIn("[string]::Equals($actualOrigin, $canonicalOrigin, [StringComparison]::OrdinalIgnoreCase)", text)
        self.assertIn("[StringComparison]::OrdinalIgnoreCase", text)
        self.assertNotIn("if ($active -ne $top)", text)
        self.assertNotIn("if ($origin -ne $canonical)", text)
