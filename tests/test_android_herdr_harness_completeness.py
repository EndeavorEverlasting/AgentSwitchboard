...
def main():
    ...
    assert '--state-root' in a['validatorIsolation']
    ...
    assert 'PASS: Android Herdr status state-root isolation' in r.stdout

    with tempfile.TemporaryDirectory() as d:
        d=Path(d)
        compat=d/'compat.md'
        r=subprocess.run([sys.executable,str(BASE/'Build-HerdrCompatibilityReview.py'),'--output',str(compat)],cwd=ROOT,text=True,capture_output=True)
        assert r.returncode==0,r.stderr
    ...
    for token in ('id: android-herdr-migration','KEEP_TMUX_HERDR_NOT_INSTALLED','Build-HerdrInstallReview.py --write','upstream-installation-source.json','state-root','same-device','Forbidden scope'):
    ...
    for token in ('test_android_herdr_status_state_isolation.py','test_android_herdr_harness_completeness.py','test_android_herdr_install_review.py','test_android_herdr_compatibility_review.py','Probe-HerdrPrebuiltCompatibility.py contract','diff --check'):
    ...
    for token in ('Test-AndroidHerdrHarnessCompleteness.ps1','test_android_herdr_status_state_isolation.py','test_android_herdr_compatibility_review.py','Build-HerdrCompatibilityReview.py','Probe-HerdrPrebuiltCompatibility.py contract','Get-HerdrHarnessStatus.py','Invoke-HerdrHarnessPreCommit.sh'):
        assert token in ci
