...
| `provider.deepseek-request` | selected route uses a `deepseek/*` provider model | apply `deepseek.usage-window.evaluate` before provider probe; block double-usage, premium, unknown, missing, stale, or unverified state |
...
The `gnhf.test-only` trigger applies even when iteration and token caps already exist. Those caps do not replace the 30-second wall-clock and per-iteration limits.
