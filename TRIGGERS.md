...
| `pi.install-request` | explicit request to install, repair, update, verify, or remove the Pi CLI | `pi.install.exact` or `pi.install.verify`; use the tracked exact-version installer, never a remembered package or floating latest command |
| `pi.single-agent-request` | explicit request to run Pi for one bounded builder lane | `pi.single-agent.route`; require exact version readiness, project trust, one writer, external artifacts, and runtime proof for provider/model response |
| `pi.opinion-fusion-request` | request for independent architect and builder opinions with adjudication | experimental `pi-fusion-orchestration` → opinion-fusion contract; preserve attribution and one designated writer; runtime remains blocked until an execution adapter is proven |
| `pi.autovalidate-request` | request for architect-owned acceptance gates followed by bounded builder repair and validator checks | experimental `pi-fusion-orchestration` → autovalidate contract; freeze gates and limits; runtime remains blocked until an execution adapter is proven |
| `pi.extension-or-package-request` | request to add a Pi extension, package, prompt pack, theme, or executable skill | separate reviewed integration sprint; pin source/version/integrity, review permissions and network behavior, test project-local loading, and preserve rollback |
...
`pi.install-request` uses only the exact package, version, minimum Node version, and install syntax recorded in `tooling/pi/harness/upstream-verification.json`. The repository installer may install the global CLI package because the user requested Pi support, but it must disable npm lifecycle scripts, verify executable and package readback, and leave global Pi settings, credentials, providers, models, hooks, and trust untouched.

`pi.single-agent-request` starts from the repository root through the pinned launcher. The launcher disables Pi install telemetry and version checks by default, may enable explicit offline mode, stores sessions and summaries outside the repository, never records raw prompts or arguments, and never passes `--approve` or changes `defaultProjectTrust`. Pi may prompt the operator to trust `.pi/settings.json`; that decision remains interactive.

The Pi CLI is free and MIT-licensed, but provider and model access is separate. A subscription, API key, local/custom provider, quota, rate, and endpoint privacy must be recorded independently. Successful installation is not provider proof.

`pi.opinion-fusion-request` and `pi.autovalidate-request` select existing bounded contracts and the experimental `pi-fusion-orchestration` skill. They remain contract-only until a tracked execution adapter proves actual executor/provider/model identity, separate role outputs, one writer, limits, cancellation, artifacts, and final delivery. Do not simulate multi-agent execution by relabeling one model response.

`pi.extension-or-package-request` is a separate code-execution review. Project-local modularity is preferred, but modular does not mean safe by default. Record exact source and integrity, permissions, filesystem/subprocess/network behavior, update mechanism, rollback, and runtime evidence before enabling it.
...
Stop or escalate when work would overwrite unowned changes; a required capability is unknown; scope crosses a forbidden boundary; writers collide; a gate exposes security or data-loss risk; merge, deployment, secret, destructive Git, or live mutation lacks authority; retries are exhausted; evidence contradicts the plan; test timing or DeepSeek gates fail; the app graph is broken; app-output context would persist raw or cross-surface data; runtime event evidence is incomplete; an end-to-end stage loses child diagnostics, effective-state readback, rollback safety, or exact environment identity; a Pi package/version is unverified, Node or npm is unavailable, project trust would be bypassed, credentials or provider state would be committed, a third-party extension is unreviewed, one route has multiple writers, role identity is unattributed, bounds are missing, or a free-access claim conflates CLI and provider cost; a launch-mode request is ambiguous, lacks a unique instance identity, creates multiple windows, or reuses one tmux session as two claimed instances; a shortcut collision is foreign, a new-instance route reuses bare `dev`, an explicit instance already exists, session allocation is exhausted, or the WezTerm command omits `--always-new-process`; or a profile has competing owners, raw frontend fallback, independent shortcut logic, cross-profile substitution, or an unproved open-or-activate claim.

## No implicit authority

The presence of a trigger, plan, startup report, event observer, topology registry, profile registry, launch-mode registry, shortcut registry, Pi installer, Pi launcher, Pi skill, app-output packet, end-to-end skill, or capability never authorizes installation, push, merge, release, deployment, target mutation, prompt execution, provider access, secret access, or destructive cleanup unless the task and repository contract explicitly allow it.
