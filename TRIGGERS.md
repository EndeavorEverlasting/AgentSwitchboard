...
| `lua.embedding-request` | Lua, embedded scripting, VM-state lifecycle, LuaJIT, host callbacks, host/script performance partition, or Lua-authored dynamic logic | `lua-embedding-integration` → `lua-task-intake`; classify host/Lua ownership and stop harness-only work before product/runtime embedding |
| `lua.sandbox-request` | untrusted, user-authored, or AI-generated Lua requests OS, file I/O, package, debug, hardware, or host capabilities | `lua-embedding-integration` → `lua-sandbox-validation`; default deny, explicit allow-list, and resource-limit gates apply |
...
| `operator.command-delivery` | copy-paste command downloads or resolves repository content, launches a child executable, installs or repairs software, crosses process boundaries, or follows an HTTP 404 / Access denied / lost-diagnostics failure | `operator-command-delivery`; resolve exact source and files, prove exact child executables, validate the actual candidate with `-CandidatePath`, preserve the parent shell, and route downstream proof honestly |
...
## Android Termux invariant

`android.termux-repo-bootstrap` selects `.ai/skills/android-termux-repo-bootstrap/SKILL.md`; `android.termux-terminal-recovery` selects `.ai/skills/android-termux-terminal-recovery/SKILL.md`. The route reads the agent contract and repository-family registry, proves a usable local clone before fetch or branch work, owns its Python validation prerequisite separately from the Android runtime installer, and does not claim repository-family readiness when its PowerShell status probe is unavailable.

Android native long-press selection, touch scrolling, and clipboard availability are not proof dependencies. The recovery route inventories exact tmux pane identities, rejects durable capture of authentication/device-code panes, captures only bounded history from one safe pane, and treats copy mode, QR, monitored documents, and files as transport alternatives rather than new authority. Static harness validation does not prove provider/model/tool behavior or repository mutation on a phone.
...
## Lua embedding invariant

`lua.embedding-request` and `lua.sandbox-request` select `.ai/skills/lua-embedding-integration/SKILL.md` and the tracked Lua workflow registry. Lua is a host-embedded library: the host owns the application main loop, performance/lifecycle-critical work, state creation/close, rollback, and cleanup. Declared isolation boundaries require independent VM states, and script errors must terminate at a protected host catch boundary.

The sandbox is fail-closed. `os`, `io`, `package`, and `debug` are denied by default; script-visible host functions require an explicit allow-list/capability manifest. Untrusted or AI-generated runtime work additionally requires defined resource limits. Lua-native 1-based indexing is preserved, and JIT remains optional and separately gated.

A harness or hosted-CI pass proves only tracked policy, workflow, fixtures, reports, and validation. Product embedding requires a separately authorized runtime sprint that pins an exact Lua implementation/version and official embedding API and then proves lifecycle, sandbox, error cleanup, resource bounds, repeated teardown, and the exact end-to-end runtime path.
...
## Operator command-delivery invariant

`operator.command-delivery` selects `.ai/skills/operator-command-delivery/SKILL.md`. The route resolves the intended ref to one exact commit, binds every required file read to that commit, proves every required external executable with a bounded process-start probe, validates the actual candidate command file with `scripts/Test-OperatorCommandDeliveryHarnessCompleteness.ps1 -CandidatePath`, and preserves the interactive parent shell.

`where`, `Get-Command`, file existence, successful download, or remembered version metadata are discovery only. A candidate containing prompt text, `$env\\:NAME`, query-string content retrieval, or any PowerShell `exit` statement is rejected. Command-delivery success cannot promote a downstream tmux, GUI, provider, deployment, or operator-acceptance claim without the downstream owning artifact.
...
Stop or escalate when work would overwrite unowned changes; a required capability is unknown; scope crosses a forbidden boundary; writers collide; a gate exposes security or data-loss risk; merge, deployment, secret, destructive Git, or live mutation lacks authority; retries are exhausted; evidence contradicts the plan; test timing or DeepSeek gates fail; the app graph is broken; app-output context would persist raw or cross-surface data; a Lua runtime sprint lacks an exact implementation/version and official embedding API, asks a harness-only lane to mutate product code, exposes ambient script capabilities, or attempts untrusted/AI-generated execution without resource limits; runtime event evidence is incomplete; an Android Termux workflow cannot identify a safe pane, lacks its Python prerequisite, has no verified local clone, or cannot obtain required repository-family status for a cross-repository claim; an operator command cannot bind source/file reads to one exact commit, cannot prove a required child executable, contains unsafe transport or `exit`, or lacks validation of the actual candidate; an end-to-end stage loses child diagnostics, effective-state readback, rollback safety, or exact environment identity; a launch-mode request is ambiguous, lacks a unique instance identity, creates multiple windows, or reuses one tmux session as two claimed instances; a shortcut collision is foreign, a new-instance route reuses bare `dev`, an explicit instance already exists, session allocation is exhausted, or the WezTerm command omits `--always-new-process`; or a profile has competing owners, raw frontend fallback, independent shortcut logic, cross-profile substitution, or an unproved open-or-activate claim.

## No implicit authority

The presence of a trigger, plan, startup report, event observer, topology registry, profile registry, launch-mode registry, shortcut registry, app-output packet, Android Termux skill, Lua embedding skill, operator-command-delivery skill, end-to-end skill, or capability never authorizes installation, push, merge, release, deployment, target mutation, prompt execution, provider access, secret access, product runtime embedding, or destructive cleanup unless the task and repository contract explicitly allow it.
