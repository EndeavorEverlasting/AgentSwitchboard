...
| `cascade.typed-validation-request` | operation/tool/event result must be structurally valid before execution and semantically valid before propagation | `typed-cascade-validation` → `typed-cascade-task-intake`; freeze contracts and correlation identity before action |
| `cascade.pre-gate-required` | operation request is ready to execute but has not passed its declared structural/type/authority contract | `typed-cascade-validation` → `typed-cascade-pre-action-validation`; reject before action on any failed check |
| `cascade.post-gate-required` | a fresh action observation exists and success propagation depends on domain/world-state coherence | `typed-cascade-validation` → `typed-cascade-post-action-validation`; enforce ontology-style rules before successor emission |
| `cascade.successor-emission-required` | both gates passed and one fresh action result may propagate | `typed-cascade-validation` → `typed-cascade-emission`; emit one immutable causally linked successor and record the sink |
...
## Typed cascade invariant

`cascade.typed-validation-request` selects `.ai/skills/typed-cascade-validation/SKILL.md` and the workflows under `tooling/cascade/harness/typed-gates/workflows/`. The input contract and ontology/domain profile are selected before action and bound to one run/action/correlation identity.

`cascade.pre-gate-required` is fail-closed. A missing required field, wrong type, out-of-range value, invalid enumeration, unknown contract, or absent declared authority rejects before execution. A pre-gate rejection cannot be reinterpreted as an action failure because no action was authorized.

`cascade.post-gate-required` runs only on a fresh observed result from the same run/action/correlation identity. Functional cardinality, disjointness, fixed enumeration, domain/range, required references, and world-state coherence are separate semantic checks. A structurally valid result may still be semantically rejected.

`cascade.successor-emission-required` requires both gate passes plus the fresh action observation. The successor uses a new event ID, inherits correlation, points causation to its immediate parent, advances sequence, and is immutable after emission. Synthetic success proves contract causality only; product runtime delivery remains a separate proof floor.
