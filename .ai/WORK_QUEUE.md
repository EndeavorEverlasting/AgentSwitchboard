...
- **Forbidden:** tracked harness mutation unless TUI exposes reproducible product defect; non-LSP grep/search semantic fallback; changing `OPENCODE_CONFIG`; free-model confidential data; committing runtime receipts; Cloud/Linux agents claiming live TUI PASS
...
- **Gate:** none
- **Last proof:** merge:24cce9e321a4913dda32f21a2d51a599dd0e4bb4 pr:#163 INTEGRATED contract floor on main (ancestor of current main); cloud/Linux runtime sprint UNPROVEN (no Windows OpenCode TUI, no `%LOCALAPPDATA%` runs, no opencode-lsp-runtime-smoke receipts); LIVE_RUNTIME_PROOF:UNPROVEN — Admin Box 1 required
- **Next action:** on Admin Box 1 only: `$ErrorActionPreference='Stop'; $repo = Join-Path $env:USERPROFILE 'dev\AgentSwitchBoard-Live'; Set-Location -LiteralPath $repo; git fetch --all --prune --tags; git checkout main; git pull --ff-only origin main; $head = (git rev-parse HEAD).Trim(); if ($head -ne '24cce9e321a4913dda32f21a2d51a599dd0e4bb4' -and -not (git merge-base --is-ancestor 24cce9e321a4913dda32f21a2d51a599dd0e4bb4 HEAD; $?)) { throw "MAIN_MISSING_ASQ005_FLOOR|head=$head" }; if (git status --porcelain) { throw 'WORKTREE_DIRTY' }; pwsh -NoLogo -NoProfile -File .\scripts\Get-RepositoryWorkLedgerFrontier.ps1 -Json; python -m unittest tests.test_asq005_canonical_runtime_floor tests.test_opencode_lsp_harness -q; $env:OPENCODE_EXPERIMENTAL_LSP_TOOL = 'true'; pwsh -NoLogo -NoProfile -File .\tooling\harness\operational\opencode-lsp-setup\Invoke-OpenCodeLspWorkstationSetup.ps1 -Mode Configure -RepoPath $repo; $run = Get-ChildItem -LiteralPath "$env:LOCALAPPDATA\AgentSwitchboard\opencode-lsp\runs" -Directory | Sort-Object LastWriteTime -Descending | Select-Object -First 1; & (Join-Path $run.FullName 'Open-AgentSwitchboard-OpenCode-Lsp.cmd'); open `tests/test_technician_live_cert_surface.py` once; LSP-only hover/definition/references on `read_text`; write local `opencode-lsp-runtime-smoke.json/.md` under $run; pwsh -NoLogo -NoProfile -File .\scripts\Test-OpenCodeLspHarness.ps1 -RootPath $repo
- **Updated:** 2026-09-13T18:05:00Z
