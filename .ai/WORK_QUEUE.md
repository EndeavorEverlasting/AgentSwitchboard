...
- **Gate:** authorized Windows Admin Box with `wsl.exe` + explicit `Ubuntu`; operator `gh auth`, passwordless apt sudo, one primary harness on non-interactive PATH, and clean FirstMate pin at `$HOME/firstmate` (or `-FirstMatePath`) when required
