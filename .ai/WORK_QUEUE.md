...
- **Last proof:** pr:#183 tip durable `Invoke-Asq017AdminBoxLiveFloor.ps1` (pending merge); cloud re-probe ContractOnly PASS / live exit 46 STATUS=BLOCKED_WINDOWS_WSL_REQUIRED PROOF_LEVEL=LIVE_ATTEMPT_FAIL_CLOSED; workers=0; LIVE_RUNTIME_PROOF:UNPROVEN — Admin Box Windows+wsl.exe+Ubuntu required; Ubuntu FirstMate pin `b182d0f908b78d08c7ccb8dce3775bdca8c5d657` + one primary harness + Ubuntu `gh` auth remain operator pre-stage for PASS; prior main@f108725 one-shot cite via #182 retained until #183 merges
