...
- **Acceptance gate:** Admin Box run reaches physical-floor PASS markers with exact-head readback and evidence root, or stops at `BLOCKED_MISSING_TOOLS` after exhausted bounded repair, or at a real non-package blocker (`BLOCKED_GITHUB_AUTH` / `BLOCKED_SUDO` / `BLOCKED_PRIMARY_HARNESS` / `BLOCKED_FIRSTMATE_DIRTY` / `BLOCKED_FIRSTMATE_PIN` / transport) with preserved evidence; cloud hosts must emit `STATUS=BLOCKED_WINDOWS_WSL_REQUIRED` exit 46 rather than unstructured crash
...
- **Last proof:** merge:0b13f9aa178bb927e74d024a577d54aae68f8050 pr:#207 INTEGRATED surface exit 44 `BLOCKED_MISSING_TOOLS` on Asq017/continuation when bounded apt repair is exhausted or fails (structured STATUS/NEXT; Admin Box paste handles 44); prior merge:2dd1040431ccb580fd7075ac006e9c74571a0e05 pr:#206 structured required-path pin blocker; cloud ContractOnly PASS / live exit 46 STATUS=BLOCKED_WINDOWS_WSL_REQUIRED PROOF_LEVEL=LIVE_ATTEMPT_FAIL_CLOSED on tip `0b13f9a`; workers=0; LIVE_RUNTIME_PROOF:UNPROVEN — Admin Box Windows+wsl.exe+Ubuntu required
