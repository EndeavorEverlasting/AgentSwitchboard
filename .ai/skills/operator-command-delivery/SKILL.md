...
8. Preserve the interactive parent shell. Do not end a command pasted at a prompt with top-level `exit`. When a child must propagate a nonzero code, run the child process, capture `$LASTEXITCODE`, print it, persist evidence, and raise/return failure without closing the parent terminal.
9. For parameterized `gh api` reads, use explicit read semantics such as `gh api --method GET <endpoint> -f ref=<commit>` instead of an interpolated content query string.
...
11. Run `pwsh -NoLogo -NoProfile -File scripts/Test-OperatorCommandDeliveryHarnessCompleteness.ps1` and `python -m unittest tests.test_operator_command_delivery_harness` before publishing a new command form when repository execution is available.
...
The positive fixture must pass. The corrupted fixture containing `$env\\:TEMP`, a PowerShell prompt prefix, query-string content retrieval, and top-level `exit` must be detected as invalid. The child-launch regression fixture must classify `Access is denied.` before any downstream artifact as `child-executable-launch-blocked`.
