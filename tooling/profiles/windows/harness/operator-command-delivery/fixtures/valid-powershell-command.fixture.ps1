...
$resolved = gh api --method GET "repos/EndeavorEverlasting/AgentSwitchboard/commits/$requestedRef" --jq '.sha'
...
if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($content)) { throw "Unable to resolve source file at $resolved." }
& cmd.exe /d /c 'ver >nul'
