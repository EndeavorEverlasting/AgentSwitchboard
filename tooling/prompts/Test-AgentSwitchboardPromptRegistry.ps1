...
Add-Check -Passed ($listRows.Count -eq 45) -Name 'selector/list-count' -Message "expected 45; got $($listRows.Count)"
Add-Check -Passed ($listRows[0].id -eq 'P00' -and $listRows[-1].id -eq 'P44') -Name 'selector/list-range' -Message 'P00-P44 not preserved'
...
try {
    ...
    $render = Invoke-Selector -Arguments @(
        ...
        'xyz_sprint_name=prompt registry proof',
        'xyz_owned_scope=prompt selector and tests',
        'xyz_forbidden_scope=provider calls',
    ...
    $gnhf = Invoke-Selector -Arguments @('-Mode', 'Show', '-PromptId', 'P38', '-ExecutionSurface', 'gnhf_launch_artifact', '-AsJson')
    $gnhfRecord = $gnhf.Text | ConvertFrom-Json
    Add-Check -Passed ($gnhfRecord.text.StartsWith('gnhf `')) -Name 'selector/gnhf-surface' -Message 'P38 is not preserved as launch content'
}
finally {
