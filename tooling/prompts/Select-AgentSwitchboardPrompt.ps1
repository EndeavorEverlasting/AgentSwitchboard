...
function ConvertTo-VariableMap {
    ...
    foreach ($record in @($Records)) {
        ...
        foreach ($part in $parts) {
            ...
            if ($separator -lt 1) {
                throw "Variable values must use name=value syntax. Invalid value: '$part'"
            }
            ...
            if (-not $KnownVariables.Contains($name)) {
                throw "Variable '$name' is not defined by the V39 prompt registry."
            }
...
if ([string]$registry.kitVersion -ne 'v39') {
    throw "Unsupported prompt-kit version: $($registry.kitVersion)"
}
$prompts = @($registry.prompts | Sort-Object sequence)
if ($prompts.Count -ne 64) {
    throw "V39 registry must contain 64 prompts; found $($prompts.Count)."
}
...
if ($Mode -in @('Show', 'Render') -and [string]::IsNullOrWhiteSpace($PromptId)) {
    throw "-$Mode requires -PromptId P00 through P63."
}
