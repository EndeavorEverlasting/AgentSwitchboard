...
9. An operator-facing result that crosses shells, child processes, WSL, tmux, WezTerm, SSH, Termux, a TUI, a GUI, or another runtime boundary selects `end-to-end-runtime-validation` after `environment-capability-routing` has classified the topology. Use `runtime-proof` for a bounded observation that does not require the complete operator path.
10. A Windows Profile request that distinguishes default open-or-activate from an explicit separate instance, or evidence of duplicate WezTerm windows, selects `windows-profile-launch-mode-validation` after the environment has been classified.
